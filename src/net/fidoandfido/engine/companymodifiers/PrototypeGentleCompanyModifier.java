...
public class PrototypeGentleCompanyModifier implements CompanyModifier {

	Logger logger = Logger.getLogger(getClass());

	public static final String NAME = "GENTLE_COMPANY_MODIFIER";
	public static long DEFAULT_MAX_REVENUE_RATE = 25;
	public static long DEFAULT_MIN_REVENUE_RATE = 8;

	public static long DEFAULT_MAX_EXPENSE_RATE = 15;
	public static long DEFAULT_MIN_EXPENSE_RATE = 5;
	...
	public boolean modifyCompanyRates(Company company) {
		...
		if (random.nextInt(CHANCE_OF_EXPENSE_CHANGE) == 0) {
			...
			if (random.nextBoolean() == true) {
				// Go up (if we can)
				if (company.getExpenseRate() < DEFAULT_MAX_EXPENSE_RATE) {
				...
				// Go down (if we can)
				if (company.getExpenseRate() > DEFAULT_MIN_EXPENSE_RATE) {
					company.modifyExpenseRate(-1);
				} else {
					company.modifyExpenseRate(1);
				}
		...
		if (random.nextInt(CHANCE_OF_REVENUE_CHANGE) == 0) {
			...
			if (random.nextBoolean() == true) {
				// Go up (if we can)
				if (company.getRevenueRate() < DEFAULT_MAX_REVENUE_RATE) {
				...
				// Go down (if we can)
				if (company.getRevenueRate() > DEFAULT_MIN_REVENUE_RATE) {
					company.modifyRevenueRate(-1);
				} else {
					company.modifyRevenueRate(1);
				}
	...
	public static void main(String argv[]) {
		...
		PrototypeGentleCompanyModifier modifier = new PrototypeGentleCompanyModifier();
